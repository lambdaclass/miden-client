...
#[test]
fn test_exec_parse() {
    let failure_script = fs::canonicalize("src/files/failure.masm").unwrap();
    let success_script = fs::canonicalize("src/files/success.masm").unwrap();
    let toml_path = fs::canonicalize("src/files/input.toml").unwrap();
